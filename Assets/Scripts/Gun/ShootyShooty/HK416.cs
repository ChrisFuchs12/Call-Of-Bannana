...
public class HK416 : NetworkBehaviour
{
    ...
    public override void OnStartClient()
    {
        base.OnStartClient();
        if (!base.IsOwner){
            GetComponent<HK416>().enabled = false;
        }
        if(base.IsOwner){
            GetComponent<HK416>().enabled = true;
        }
    }
    ...
    [ServerRpc]
    public void SpawnObject(GameObject obj, Transform player, HK416 script)
    {
    ...
    [ObserversRpc]
    public void SetSpawnedObject(GameObject spawned, HK416 script)
    {
        spawned.GetComponent<Rigidbody>().velocity = bulletSpawnPoint.forward * bulletSpeed;
